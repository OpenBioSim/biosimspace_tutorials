...
ligand_1_sys = BSS.IO.readMolecules(
    [
        f"{ligs_path}{lig_0}_lig_equil_solv.rst7",
        f"{ligs_path}{lig_0}_lig_equil_solv.prm7",
    ]
)
ligand_2_sys = BSS.IO.readMolecules(
    [
        f"{ligs_path}{lig_1}_lig_equil_solv.rst7",
        f"{ligs_path}{lig_1}_lig_equil_solv.prm7",
    ]
)
...
system_1 = BSS.IO.readMolecules(
    [
        f"{ligs_path}{lig_0}_sys_equil_solv.rst7",
        f"{ligs_path}{lig_0}_sys_equil_solv.prm7",
    ]
)
system_2 = BSS.IO.readMolecules(
    [
        f"{ligs_path}{lig_1}_sys_equil_solv.rst7",
        f"{ligs_path}{lig_1}_sys_equil_solv.prm7",
    ]
)
...
if not num_lambda:
    raise NameError(
        f"The perturbation {lig_0}~{lig_1} (or the reverse) was not found in network.dat."
    )
...
if engine_query == "GROMACS":
    ...
    npt_protocol = BSS.Protocol.FreeEnergyEquilibration(
        num_lam=num_lambda, pressure=1 * BSS.Units.Pressure.atm
    )
